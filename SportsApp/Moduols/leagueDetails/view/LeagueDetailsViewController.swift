...
class LeagueDetailsViewController: UICollectionViewController {
    ...
    func addToFavourites(viewModel : LeagueDetailsViewModel) {
        if viewModel.isFavoutite {
    ...
    // Sticky cell logic
        override func scrollViewDidScroll(_ scrollView: UIScrollView) {
            let stickyHeaderIndexPath = IndexPath(item: 0, section: 0)
            guard let stickyHeader = collectionView.supplementaryView(forElementKind: UICollectionView.elementKindSectionHeader, at: stickyHeaderIndexPath) else {
                return
            }

            let offsetY = scrollView.contentOffset.y
            let stickyCellHeight: CGFloat = 50 // Adjust to your sticky cell's height
            let topInset = collectionView.adjustedContentInset.top

            if offsetY < 0 {
                stickyHeader.frame.origin.y = topInset
            } else {
                stickyHeader.frame.origin.y = min(topInset, offsetY + topInset)
            }
        }
...
//        case 0:
//            sectionHeaderView.SectionHeaderLabel.text = viewModel?.league.league_name
//            sectionHeaderView.SectionHeaderLabel.font = UIFont.boldSystemFont(ofSize: 22)
//            sectionHeaderView.vc = self
//            sectionHeaderView.vm = self.viewModel
//            if (viewModel?.isFavoutite)! {
//                sectionHeaderView.favouriteButton.imageView?.image = UIImage(systemName: "heart.fill")
//            }
//            sectionHeaderView.backButton.isHidden = false
//            sectionHeaderView.favouriteButton.isHidden = false
//            sectionHeaderView.backgroundColor = UIColor.white
//            return sectionHeaderView
//        case 1:
//            sectionHeaderView.SectionHeaderLabel.text = "UpComing"
//            sectionHeaderView.SectionHeaderLabel.font = UIFont.boldSystemFont(ofSize: 22)
//            sectionHeaderView.backButton.isHidden = true
//            sectionHeaderView.favouriteButton.isHidden = true
